...
public sealed class CodeGenerationStepHandler(
        ...
        CancellationToken cancellationToken)
    {
        var llmResponse = await llmGateway.GenerateAsync(
            new LlmRequest(
                Messages:
                [
                    new LlmMessage(LlmMessageRole.System, "You are MAACO code generation step."),
                    new LlmMessage(LlmMessageRole.User, $"Generate minimal patch for workflow {context.WorkflowId:D}")
                ],
