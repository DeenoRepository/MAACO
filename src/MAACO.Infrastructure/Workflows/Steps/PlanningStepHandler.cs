...
public sealed class PlanningStepHandler(
    ILlmGateway llmGateway,
    ILogRepository logRepository) : IWorkflowStepHandler
{
    public string Name => "PlanningStep";
        ...
        CancellationToken cancellationToken)
    {
        var llmResponse = await llmGateway.GenerateAsync(
            new LlmRequest(
                Messages:
                [
                    new LlmMessage(LlmMessageRole.System, "You are MAACO planning step."),
                    new LlmMessage(LlmMessageRole.User, $"Plan workflow {context.WorkflowId:D}")
                ],
